...
class ImageBuffer():
    def __init__(
        ...
        self.next_state_memory = np.zeros((self.memory_size, self.trajectory_length, 3, self.state_dim, self.state_dim), dtype=float)
        self.state_memory = np.zeros((self.memory_size, self.trajectory_length, 3, self.state_dim, self.state_dim), dtype=float)

        self.f = []
        f_ptr = h5py.File(self.filepath + str(self.iter) + 'mem.hdf5', 'w') #, HDF5_USE_FILE_LOCKING='FALSE')
        self.sim_data = f_ptr.create_group('sim')
        # state includes: gripper xyz, (block xyz, dx dy dz) x 3 blocks, gleft, gright, z joint: 24
        self.sim_data.create_dataset('states', (self.memory_size, self.trajectory_length, 3, self.state_dim, self.state_dim), dtype='f')
        self.sim_data.create_dataset('next_states', (self.memory_size, self.trajectory_length, 3, self.state_dim, self.state_dim), dtype='f')
    ...
    def add_sample(self, states, next_states, actions):
        ...
        with h5py.File(self.filepath + str(self.iter) + 'mem.hdf5', 'a') as f_ptr:
